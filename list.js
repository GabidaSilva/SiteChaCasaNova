...
const gifts = [
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelectorAll(".pix-btn").forEach(button => {
            button.addEventListener("click", handleContribution);
        });
    });
    ...
    async function handleContribution(event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                throw new Error(data.message || "Erro ao registrar a contribuição.");
            }
        } catch (error) {
            console.error('Erro:', error);
            alert(error.message || "Ocorreu um erro ao processar a contribuição.");
        }
    }
    ...
    async function updateContributionTotal(giftId, price) {
        try {
            ...
            if (!response.ok) {
                throw new Error("Falha ao buscar contribuições");
            }
            const data = await response.json();
